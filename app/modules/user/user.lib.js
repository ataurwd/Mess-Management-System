...
class UserLib {
    ...
    static async updateProfile(id, updateObject) {
        try {
            ...
            delete updateObject.messId;

            if (updateObject.password && typeof updateObject.password === 'string' && updateObject.password.trim() !== '') {
                updateObject.password = bcrypt.hashSync(updateObject.password, saltRounds);
            } else {
                delete updateObject.password;
            }
            return await UserModel.findByIdAndUpdate(id, { $set: updateObject }, { new: true });
        } catch (e) {
            throw e;
        }
    }
    ...
    static async updateUser(userId, updateObject) {
        try {
            ...
            return await UserModel.findByIdAndUpdate(userId, { $set: updateObject }, { new: true });
        } catch (e) {
            throw e;
        }
    }
