...
module.exports = class AuthValidation {
    ...
    static addUserValidation(req, res, next) {
        if (!req.body.username || String(req.body.username).trim().length === 0) return res.status(200).send({ success: false, message: "User name can't be empty" });
        if (!req.body.email || String(req.body.email).trim().length === 0) return res.status(200).send({ success: false, message: "Email can't be empty" });
        if (!req.body.password || String(req.body.password).trim().length === 0) return res.status(200).send({ success: false, message: "Password can't be empty" });
        next();
    }
