...
class UserController {
    ...
    static async updateProfile(req, res) {
        try {
            ...
            const targetId = (req.auth && req.auth.role === 'admin' && updateObject.userId) ? updateObject.userId : req.auth.id;
            delete updateObject.userId;
            const data = await UserLib.updateProfile(targetId, updateObject);
    ...
    static async updateUser(req, res) {
        try {
            ...
            const updateObject = { ...req.body };
            delete updateObject.userId;
            const data = await UserLib.updateUser(userId, updateObject);
            return res.status(200).json(response.single(true, 'User updated successfully', data));
        } catch (e) {
            return res.status(500).json(response.error(false, 'An error occur', `${e}`));
        }
    }
