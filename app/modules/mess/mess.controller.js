...
class MessController {
    ...
    static async messSummary(req, res) {
        try {
            const firstDate = (req.body && req.body.firstDate) || (req.query && req.query.firstDate) || currentMonthFirstDate;
            const lastDate = (req.body && req.body.lastDate) || (req.query && req.query.lastDate) || currentMonthLastDate;
            const summary = await MessLib.messSummary(firstDate, lastDate, req.auth.messId);
