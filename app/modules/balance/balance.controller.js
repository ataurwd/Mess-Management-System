...
class BalanceController {
    static async addBalance(req, res) {
        try {
            ...
            if (balanceObject.date) {
                balanceObject.date = new Date(balanceObject.date).toISOString();
            } else {
                balanceObject.date = new Date().toISOString();
            }
            if (!balanceObject.categoryId || balanceObject.categoryId === '') {
                delete balanceObject.categoryId;
            }

            const balance = await BalanceLib.addBalance(balanceObject);
            return res.status(201).json(response.single(true, balance.message, balance.data));
        } catch (e) {
            return res.status(500).json(response.error(false, 'An error occur', `${e}`));
        }
    }

    static async getMessBalances(req, res) {
        try {
            const { messId } = req.auth;
            const balance = await BalanceLib.getMessBalances(messId);
            return res.status(200).json(response.single(true, 'Mess Balances', balance.data || []));
        } catch (e) {
            return res.status(500).json(response.error(false, 'An error occur', `${e}`));
        }
    }
